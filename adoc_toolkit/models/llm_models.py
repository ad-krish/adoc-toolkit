...
class LLMRequest(BaseModel):
    ...
    additional_params: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional vendor-specific parameters")
...
class ClaudeRequest(LLMRequest):
    ...
    max_tokens: int = Field(default=4000, description="Maximum tokens for Claude response")
...
class ChatGPTRequest(LLMRequest):
    ...
    max_tokens: int = Field(default=4000, description="Maximum tokens for ChatGPT response")
    temperature: float = Field(default=0.7, description="Temperature for ChatGPT response generation")
...
class LLMClientConfig(BaseModel):
    ...
    additional_config: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional vendor-specific configuration")
...
class PromptConfig(BaseModel):
    ...
    system_prompt_text: Optional[str] = Field(default=None, description="System prompt text")
...
class DQPolicyPromptConfig(PromptConfig):
    ...
    system_prompt_file: str = Field(default="config/prompts/text_to_dq_policy_system.txt", description="Path to DQ policy system prompt file")
    user_prompt_template: str = Field(default="Provided text content:\n{text}", description="User prompt template for DQ policy generation") 