...
def safe_get(container: Any, key: str, default: Any = None) -> Any:
    ...
    try:
        return (
            getattr(container, key, default)
            if hasattr(container, key)
            else container.get(key, default)
        )
...
def create_llm_response(
    ...
    return LLMResponse(
        content=content, model_used=model, vendor=vendor, metadata=metadata
    )
...
class BaseLLMClient(ABC):
    ...
    def _add_processing_time(
        self, response: LLMResponse, start_time: float
    ) -> LLMResponse:
...
class GrokLLMClient(BaseLLMClient):
    ...
    def _generate_grok_response(self, request: LLMRequest) -> LLMResponse:
        ...
        return create_llm_response(
            ...
            vendor="grok",
            metadata={
                "timeout": grok_request.timeout,
                "temperature": grok_request.temperature or 0.2,
            },
        )
...
class GeminiLLMClient(BaseLLMClient):
    ...
    def _generate_gemini_response(self, request: LLMRequest) -> LLMResponse:
        ...
        return create_llm_response(
            ...
            vendor="gemini",
            metadata={
                **gemini_request.additional_params,
    ...
    def _create_gemini_messages(self, gemini_request: GeminiRequest) -> list:
        ...
        combined_prompt = (
            f"{gemini_request.system_prompt}\n\n{gemini_request.user_prompt}"
        )
        return [{"role": "user", "parts": [combined_prompt]}]
...
class ClaudeLLMClient(BaseLLMClient):
    ...
    def _create_claude_messages(self, claude_request: ClaudeRequest) -> list:
        ...
        return [{"role": "user", "content": claude_request.user_prompt}]
