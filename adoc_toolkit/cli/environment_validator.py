...
class EnvironmentValidator:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if value in stripped_line:
                ...
                if len(field_parts) == 3:  # environments.env.field
                    ...
                    if f"{field_name}:" in stripped_line and env_name in "".join(
                        lines[:i]
                    ):
                        return i
    ...
    def validate_config(self) -> tuple[bool, list[str]]:
        ...
        environment_errors = self._validate_environments(
            config_data.get("environments", {})
        )
    ...
    def _validate_environment_fields(
        ...
        if not isinstance(name_value, str):
            line_num = self._find_line_number(
                f"environments.{env_name}.name", str(name_value)
            )
            errors.append(
            ...
            line_num = self._find_line_number(
                f"environments.{env_name}.name", name_value
            )
            errors.append(
        ...
        if not isinstance(base_url, str):
            line_num = self._find_line_number(
                f"environments.{env_name}.base_url", str(base_url)
            )
            errors.append(
            ...
            if not base_url.startswith(("http://", "https://")):
                line_num = self._find_line_number(
                    f"environments.{env_name}.base_url", base_url
                )
                errors.append(
        ...
        if not isinstance(access_key, str):
            line_num = self._find_line_number(
                f"environments.{env_name}.access_key", str(access_key)
            )
            errors.append(
            ...
            if not re.match(r"^[A-Z0-9]+$", access_key):
                line_num = self._find_line_number(
                    f"environments.{env_name}.access_key", access_key
                )
                errors.append(
        ...
        if not isinstance(secret_key, str):
            line_num = self._find_line_number(
                f"environments.{env_name}.secret_key", str(secret_key)
            )
            errors.append(
            ...
            if not re.match(r"^[A-Z0-9]+$", secret_key):
                line_num = self._find_line_number(
                    f"environments.{env_name}.secret_key", secret_key
                )
                errors.append(
    ...
    def _validate_default_environment(self, config_data: dict[str, Any]) -> list[str]:
        ...
        if "default_environment" in config_data:
            ...
            if not isinstance(default_env, str):
                line_num = self._find_line_number(
                    "default_environment", str(default_env)
                )
                errors.append(
