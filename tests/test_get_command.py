...
class TestGetCommand:
    ...
    def test_command_description(self):
        ...
        assert (
            self.command.description == "Make HTTP GET requests to ADOC API endpoints"
        )
    ...
    def test_parse_query_params_float(self):
        ...
        assert (
            params["ratio"] == "0.5"
        )  # Floats are kept as strings in simplified version
    ...
    def test_execute_with_query_params(self, mock_print_response, mock_load_api):
        ...
        with patch.object(self.command.console, "print"):
            ...
            self.http_client.get.assert_called_once_with(
                "/test", params={"param1": "value1", "param2": "value2"}
            )
        ...
        mock_endpoint = APIEndpoint(
            ...
            query_params={
                "format": QueryParameter(
                    type="string", description="Format", options=["json", "xml"]
                )
            },
            response_type="json",
        )
    ...
    def test_get_command_args_validation(self):
        ...
        # Test valid args
        args = GetCommandArgs(
            endpoint="/test", query_params={"param": "value"}, path_params={"id": "123"}
        )
    ...
    def test_get_completions_path_parameters(self, mock_load_api):
        ...
        mock_api_ref.endpoints = {
            "/catalog-server/api/assets/:asset-id/metadata": mock_endpoint
        }
        mock_load_api.return_value = mock_api_ref
    ...
    def test_get_completions_path_parameters_multiple(self, mock_load_api):
        ...
        completions = self.command.get_completions(
            "get /catalog-server/api/assets/:asset-id/users/:user-id/permissions ", 70
        )
        assert len(completions) == 2
        assert "asset-id" in completions
    ...
    def test_get_completions_url_partial(self, mock_load_api):
        ...
        completions = self.command.get_completions(
            "get /catalog-server/api/assets/", 30
        )
        assert len(completions) == 2
    ...
    @patch.object(GetCommand, "_load_api_reference")
    @patch("adoc_toolkit.http.formatter.ResponseFormatter.print_response_with_config")
    def test_execute_honors_response_type_config(
        self, mock_print_response, mock_load_api
    ):
