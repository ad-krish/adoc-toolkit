...
def test_grok_generate_response_import_error(dummy_console):
    ...
    with patch.object(c, '_generate_grok_response', side_effect=ImportError("No module named 'xai_sdk'")):
...
def test_gemini_generate_response_import_error(dummy_console):
    ...
    with patch.object(c, '_generate_gemini_response', side_effect=ImportError("No module named 'google.generativeai'")):
        with pytest.raises(ImportError):
            c.generate_response(request)
...
def test_claude_generate_response_import_error(dummy_console):
    ...
    with patch.object(c, '_generate_claude_response', side_effect=ImportError("No module named 'anthropic'")):
        with pytest.raises(ImportError):
            c.generate_response(request)
...
def test_dq_policy_llm_client_generate_with_grok(dummy_console):
    ...
    client = llm_client.DQPolicyLLMClient(mock_llm_client, dummy_console, test_processor)
